...
def test_student_biometrics_and_location_persistence():
    print("\n=== STARTING STUDENT BIOMETRICS & LOCATION PERSISTENCE REGRESSION SUITE ===")

    # 0. Setup test faculty and test student credentials
    ts = int(time.time())
    test_faculty_email = f"fac_bio_p_{ts}@college.edu"
    test_student_email_a = f"stu_bio_pa_{ts}@college.edu"
    test_student_email_b = f"stu_bio_pb_{ts}@college.edu"
    test_student_id_a = f"STU_BIO_PA_{ts}"
    test_student_id_b = f"STU_BIO_PB_{ts}"
    ...
    # 4. Mandatory Password Change for Student A
    ...
    if stu_a_data.get("mustChangePassword"):
        ch_a_res = requests.put(f"{GATEWAY_URL}/api/auth/password", headers=stu_a_headers, json={
            "currentPassword": password,
            "newPassword": "NewStudentPassword123!"
        })
        assert ch_a_res.status_code == 200
    ...
    try:
        ...
        # TEST 1 — First Enrollment: Face Enrollment
        ...
        # TEST 5 — Duplicate Face Enrollment (must update in place without duplicate)
        ...
        # TEST 1 — First Enrollment: Device Fingerprint Enrollment
        ...
        # TEST 5 — Duplicate Device Enrollment (must update in place)
        ...
        # Cross-Account Device Protection Check (Student B attempts to bind Student A's device -> 409 Conflict)
        stu_b_login_res = requests.post(f"{GATEWAY_URL}/api/auth/login", json={
            "email": test_student_email_b,
            "password": "NewFacultyPassword123!"
        })
        assert stu_b_login_res.status_code == 200, f"Student B login failed: {stu_b_login_res.text}"
        stu_b_data = stu_b_login_res.json()
        stu_b_token = stu_b_data["token"]
        stu_b_headers = {"Authorization": f"Bearer {stu_b_token}"}

        if stu_b_data.get("mustChangePassword"):
            ch_b_res = requests.put(f"{GATEWAY_URL}/api/auth/password", headers=stu_b_headers, json={
                "currentPassword": "NewFacultyPassword123!",
                "newPassword": "NewStudentBPassword123!"
            })
            assert ch_b_res.status_code == 200
            stu_b_login_res = requests.post(f"{GATEWAY_URL}/api/auth/login", json={
                "email": test_student_email_b,
                "password": "NewStudentBPassword123!"
            })
        ...
        # TEST 2 — Logout / Login & Auth Me check
        ...
        # TEST 3 & 9 — Refresh 10 times & No Automatic Creation Check
        for _ in range(10):
        ...
        with conn_stu.cursor(dictionary=True) as cursor:
            ...
            cursor.execute("SELECT count(*) as cnt FROM device_enrollments WHERE student_id = %s", (stu_a_db_id,))
            final_dev_cnt = cursor.fetchone()["cnt"]
            assert final_dev_cnt == 1, f"Refreshes triggered automatic creation! Count: {final_dev_cnt}"

        # TEST 6, 7 & 8 — Location Verification Tests
        subj_res = requests.post(f"{GATEWAY_URL}/api/faculty/subjects", headers=fac_headers, json={
            "subjectCode": "SUB_BIO_P101",
        ...
        # TEST 7 — Location Outside Radius
        ...
        print("=== ALL PERSISTENCE AND LOCATION REGRESSION TESTS PASSED 100% ===")
        ...
        with conn_fac.cursor(dictionary=True) as cursor:
            cursor.execute("DELETE FROM subjects WHERE subject_code = 'SUB_BIO_P101'")
            cursor.execute("DELETE FROM faculty WHERE email = %s", (test_faculty_email,))

if __name__ == "__main__":
    test_student_biometrics_and_location_persistence()
